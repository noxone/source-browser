...
Scan phase  ┌──────────────────────────────────────────────────────────────┐
            ...
            │                                                              │
            │  merged ──► token stream (gzip JSON, stored in DB)          │
            └──────────────────────────────────────────────────────────────┘

Click phase ┌──────────────────────────────────────────────────────────────┐
            │  token click ──► GET /api/lsp/hover?fileId=&line=&column=    │
            │               ──► LSP hover + definition ──► right panel    │
            └──────────────────────────────────────────────────────────────┘
```
...
### 3. Install the TypeScript Language Server
...
|---|---|---|
| `sourceviewer.treesitter.grammars.dir` | `TREESITTER_GRAMMARS_DIR` | `/opt/treesitter-grammars` |
...
- LSP servers are started **lazily** on the first scan of a given language in a repository.
- The same server instance is reused for subsequent scans **and** for interactive hover queries.
- Servers idle for more than `sourceviewer.lsp.inactive-timeout-minutes` (default 60) are shut down automatically.
- On application shutdown all servers are stopped gracefully.
