<template>
  <div class="h-full flex flex-col">
    ...
    <div class="flex-1 min-h-0 overflow-y-auto px-8 pb-6">
      ...
      <div v-else>
        ...
        <div class="mb-2 text-sm text-gray-500">
          <span class="font-medium text-gray-700">{{ results.length }}</span>
          {{ results.length === 1 ? 'result' : 'results' }}
          <span v-if="results.length >= MAX_RESULTS" class="text-gray-400"> &mdash; showing first {{ MAX_RESULTS }}, try a more specific query</span>
          <span v-else> for <span class="font-medium text-gray-600">{{ lastQuery }}</span></span>
        </div>
    ...
    <div
      v-if="indexStats"
      class="shrink-0 px-8 py-2 bg-white border-t border-gray-200 flex flex-wrap gap-x-6 gap-y-1 text-xs text-gray-500"
    >
...
async function runSearch() {
  ...
  try {
    results.value = await search(q, MAX_RESULTS, 0, selectedRepoIds.value, fileFilterForUrl)
  } catch (err) {
